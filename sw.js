const CACHE = 'metanoia-2026-v3';   // v3 : purge du cache pollue par les reponses API
...
self.addEventListener('fetch', e => {
  ...
  // ⚠️ REGRESSION CORRIGEE : la condition « pas mon origine » attrapait AUSSI
  // les appels a l'API Supabase (REST, RPC, Realtime), qui vivent sur un autre
  // domaine. Resultat : les messages de guilde etaient servis depuis le cache,
  // donc perimes — un message envoye par un autre compte n'apparaissait jamais.
  // On ne met desormais en cache QUE les scripts et polices de CDN connus.
  var _cdnOk = /^https:\/\/(cdn\.jsdelivr\.net|fonts\.googleapis\.com|fonts\.gstatic\.com|unpkg\.com)\//.test(req.url);
  if (req.method === 'GET' && _cdnOk) {
    e.respondWith(
